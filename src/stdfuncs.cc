...
#define MATH_FUNC(name, func) context.symbols->addFunction(name, [&](std::vector<sharedObj> arg) {\
...
void addStdFunctions(const Context& context) {
    context.symbols->addFunction("print", [&](std::vector<sharedObj> arg) {
        std::string res;
        for (auto i : arg) {
            std::cout << i->toString() << " ";
            res += i->toString();
    ...
    context.symbols->addFunction("str", [&](std::vector<sharedObj> arg) {
        return arg[0]->toString();
    }, 1);
    context.symbols->addFunction("input", [&](std::vector<sharedObj> arg) {
        std::string line;
        std::getline(std::cin, line);
        return line;
    }, 0);
    context.symbols->addFunction("getnum", [&](std::vector<sharedObj> arg) {
        int num;
        if (!scanf("%d", &num)) {
    ...
    context.symbols->addFunction("getword", [&](std::vector<sharedObj> arg) {
        std::string s;
        std::cin >> s;
        return s;
    }, 0);

    context.symbols->addFunction("int", [&](std::vector<sharedObj> arg) {
